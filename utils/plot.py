...
def analyze_results(results: dict, output_dir: str = ".", output_plot_name: str = "refusal_analysis.png"):
    ...
    # Plot 1: Mean Norms
    ax1 = axes[0, 0]
    ax1.plot(layer_indices, harmful_norms, 'r-o', label='Harmful Mean', linewidth=2, markersize=4)
    ax1.plot(layer_indices, harmless_norms, 'g-s', label='Harmless Mean', linewidth=2, markersize=4)
    ax1.plot(layer_indices, refusal_directions, 'b-^', label='Refusal Direction', linewidth=2, markersize=4)
    ...
    ax2 = axes[0, 1]
    ax2.plot(layer_indices, cosine_similarities, 'purple', label="Harmful to harmless", marker='o', linewidth=2, markersize=4)
    ax2.plot(layer_indices, cosine_similarities_harmful, 'red', label="Harmful to refusal", marker='o', linewidth=2, markersize=4)
    ax2.plot(layer_indices, cosine_similarities_harmless, 'blue', label="Harmless to refusal", marker='o', linewidth=2, markersize=4)
    ...
    ax3 = axes[1, 0]
    ax3.plot(layer_indices, snratios, 'darkorange', label="Signal to noise", marker='d', linewidth=2, markersize=4)
    ax3.plot(layer_indices, purity_ratios, 'darkgreen', label="Refusal purity", marker='d', linewidth=2, markersize=4)
    ...
    ax4 = axes[1, 1]
    ax4.plot(layer_indices, signal_quality_estimates, 'teal', marker='*', linewidth=2, markersize=6)
