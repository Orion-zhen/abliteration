...
def resolve_model_paths(
    model_id: str,
) -> Tuple[Optional[str], Union[Path, str], Dict[str, str], List[str]]:
    """
    ...
    if not index_path:
        ...
        if single_file:
            ...
            if (local_path / "model.safetensors.index.json").exists():
                ...
                raise ValueError(
                    "Could not find model.safetensors.index.json or model.safetensors"
                )
...
def copy_model_artifacts(
    config: ModelConfig, output_path: Path, index_path: Optional[str]
):
    """
...
def load_measurements(path: str) -> tuple[dict, dict]:
    ...
    )  # weights_only=False primarily for dict structure support, trusted source assumption
